"""
Editted version of the Pastas project in which a forecast function is added,
see bottom of file 

(Karl Schutt)
-----

The original module is created at Artesia Water by Raoul Collenteur.
...
from pastas.project.maps import Map
from pastas.project.plots import Plot
from pastas.decorators import PastasDeprecationWarning
from pastas.io.base import dump
from pastas.model import Model
from pastas.rfunc import Gamma
from pastas.stressmodels import StressModel2
...
class Project:
    ...
    def __init__(self, name, metadata=None):
        ...
        ###
        self.forecast_stresses = pd.DataFrame(columns=columns)
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    ...
    # New functionality for performing a forecast for multiple models in a pastas project
    
    def forecast(self,stresses):
        """ By calling this function a forecast is performed for a specified
        length. 
        
        Parameters
        -----------
        Stresses: pandas dataframe 
            Dataframe with stress name as index that contains the timeseries 
            of the stresses (future values for forecasting) in a column named "series"
            and the kind of the series in a column named "kind".
        """

        # Backup historic series
        Backup = self.stresses.copy()
                    
        # Import the precipitation timeseries in the Pastas project
        for i in range(len(stresses.index)): 
            name = stresses['series'][i].name
            metadata =  stresses['series'][i].metadata
            kind = stresses['kind'][i]
            # Keep historical series
            series_hist = Backup['series'][name].series
            self.del_stress(name)
            series_forc = stresses['series'][i].series
            new_series = TimeSeries(series_hist.append(series_forc),name = name, metadata = metadata)
            self.add_series(new_series,kind=kind, settings = kind) 
        
        self.update_model_series()
        
        # Now perform the forecasts:
        
        d = dict(keys = self.oseries.index)
        
        
        for i in range(len(self.oseries.index)):
            name = self.oseries.index[i]
            print(name)
            ml = self.models[name].copy()
            tmax = stresses['series'][0].series.index[len(stresses['series'][0].series.index)-1]
            tmin = stresses['series'][0].series.index[0]
            ml.settings['tmax']=tmax
            p = ml.simulate()[tmin:tmax]

            p = pd.DataFrame(p)
            d[name]=p
        
        self.stresses = Backup # Set stresses back to old values
        self.update_model_series()
        
        return(d) # Dictionary that contains forecasts of the different models
